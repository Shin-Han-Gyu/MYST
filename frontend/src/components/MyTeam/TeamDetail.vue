<template>
  <div class="bg-secondary container">
    <div style="text-align: center;" class="text-white team-name">
      {{ team.name }}
    </div>
    <p></p>
    <div style="text-align: center;" class="text-white team-content">
      {{ team.content }}
    </div>
    <p></p>
    ...
    <ul class="list-group border border-3">
      <li
        class="border-warning bg-secondary d-flex justify-content-around list-group-flush"
        v-for="member in team.groupMemberDtos"
        :key="member"
      >
    ...
    <button
      type="button"
      v-if="info.position == 'Leader'"
      class="btn btn-btn"
      data-bs-toggle="modal"
      data-bs-target="#joinlist"
    >
      가입요청 확인
    </button>
    <br />
    <button v-if="info.position == 'None'" class="btn btn-btn" @click="join()">
      가입신청
    </button>
    <br />
      ...
      <div class="card" :style="{ 'background-color': todo.colorCode }">
        <div class="card-body bg-warning">
          <div class="d-flex justify-content-between">
            <div>
              <p class="todotitle" :style="{ color: fontColor }">
                {{ todo.taskName }}
              </p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    checkTodo: function(taskId) {
      this.$store.dispatch("todo/checkTodo", {
        taskId: taskId,
        token: this.$store.state.login.userinfo.userToken,
      });
      location.reload();
    },
    join: function() {
