      ...
      <div class="card-body">
        <div class="d-flex justify-content-between">
          <div>
            ...
            <p v-else class="todotitle" :style="{ color: fontColor }">
              {{ todo.taskName }}
            </p>
            <p
              class="todoteam"
            ...
            <p class="todoteam" :style="{ color: fontColor }" v-else>
              {{ todo.teamName }}
            </p>
          </div>
          <div style="text-align: center;">
            ...
            <i
              v-else
              :style="{ color: fontColor }"
              class="far fa-check-square checkbox"
              @click="checkTodo()"
            ></i>
            <br />
...
export default {
  ...
  created() {
    ...
    if (this.todo.teamId == -1 && this.todo.taskDone === "N") {
      ...
      this.todo.done.includes(this.$store.state.login.userinfo.username)
    ) {
      this.taskDone = true;
    } else {
      this.taskDone = false;
